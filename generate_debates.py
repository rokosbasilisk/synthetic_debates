...
prompts = {
        ...
        "human_judge": """
        You are a human judge overseeing a debate.
        """,
...
def run_debate(topic):
    ...
    debate_motion = prompts["debate_motion"].replace("TOPIC", topic).replace("ALICE_POSITION", positions[0]).replace("BOB_POSITION", positions[1])
...
def run_debates_multiprocessed(topic_list, max_processes=4):  
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate Debates Script")
    parser.add_argument("--input_topic_filename", type=str, required=True, help="Path to the input topics file")
    parser.add_argument("--output_debate_filename", type=str, required=True, help="Path to the output debates file")
    args = parser.parse_args()

    process_topics_and_debates(args.input_topic_filename, args.output_debate_filename)
